...
## 株式会社アクティブゲーミングメディア様の依頼で制作したプラグイン
- プラグインをいくつか
...
## 冬空橙様の依頼で修正したプラグイン
- エラーの修正
  - あるプラグインが原因で発生していたエラーを抑制するプラグインの作成（2種類）

## 令和用牧場主様の依頼で修正したプラグイン
- 不具合の修正
  - あるプラグインが正常に動作しなくなっていたため、修正（コアスクリプトの更新の影響と思われる）。

## 甘納豆様の依頼で作成したプラグイン
- 並び替えプラグインの改変
  - ゲームに合わせてデザインを変更
- 熟練度システムの作成
  - 熟練度システムを新規に作成。熟練度が溜まったスキルは次のスキルへ進化

## ぐりべる様の依頼で作成したプラグイン
- バトル時に徐々にHPが減るシステム
  - M○ther2みたいな感じのやつ
  - MOG系のバトルプラグインと組み合わせて使える

## 株式会社コミックウェア様の依頼で作業した内容
- 技術的アドバイス
  - サーバーからデータを正常に取得できない原因の調査と対策

## コロナ様の依頼で作成したプラグイン
 - 戦闘敗北時の処理を変更
   - 特定のアイテムを持っているとき、復活するかどうかを選べる
   - 復活時はアイテムを消費
   - バトル画面の場合はそのまま戦闘続行
   - 参考URL：https://kimitu-rosutobaisu.jimdofree.com/
